...
int flatcc_builder_default_alloc(void *alloc_context, iovec_t *b, size_t request, int zero_fill, int hint)
{
    ...
    if (zero_fill && b->iov_len < n) {
        (void)memset((uint8_t *)p + b->iov_len, 0, n - b->iov_len);
    }
    b->iov_base = p;
    b->iov_len = n;
    return 0;
...
{
    B->ds_offset -= size;
    (void)memset(B->ds + B->ds_offset, 0, size);
}
...
void flatcc_builder_flush_vtable_cache(flatcc_builder_t *B)
{
    ...
    (void)memset(buf->iov_base, 0, buf->iov_len);
...
{
    (void)memset(B->ds, 0, B->ds_offset);
